...
<?php
   ...
   require_once "clean.php" ;
    ...
    $fnameError = $lnameError = $emailError = $passError = $phoneError = $addressError = ""; 
   ...
   if(isset ($_POST["register"])){
        ...
        // simple validation for the "phone"
        if ( empty ($phone)){
           $error = true ;
       ...
       if ( empty ($address)){
        ...
        if (!$error){ 
           ...
           $sql = "INSERT INTO users (`first_name`, `last_name`, `email`, `phone_number`, `photo`, `address`, `password`) VALUES ('$fname','$lname', '$email', '$phone', '$photo[0]', '$address', '$password')" ;

           $result = mysqli_query($connect, $sql);

            if ($result){
                echo   "<div class='alert alert-success'>
               <p>New account has been created, $photo[1]</p>
           </div>" ;
